...
# Exact Swag Labs copy (h3[data-test=error]). Empty username is checked
# first, so both-empty uses the username-required message.
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("unknown_user", "secret_sauce", BAD_CREDENTIALS),
...
def test_login_shows_exact_error(login_page: LoginPage, username, password, error):
    login_page.login_user(username, password)
    assert login_page.get_error_message().inner_text().strip() == error